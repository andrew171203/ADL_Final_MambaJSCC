...
import torch
from net.mamba_adapter import VSSBlockAdapter
from net.model_names import normalize_model_name

class LegacySwinTransformerBlock(nn.Module):
...
SwinTransformerBlock = LegacySwinTransformerBlock
...
class BasicLayer(nn.Module):
    def __init__(self, dim, out_dim, input_resolution, depth, num_heads, window_size,
        ...
        self.blocks = nn.ModuleList([
            # LegacySwinTransformerBlock(dim=out_dim,
...
class VSSBJSCC_Encoder(nn.Module):
    ...
    def forward(self, x, snr, rate, model):
        ...
        if model == 'VSSBJSCC_w/o_SAandRA':
            x = self.head_list(x)
            return x

        elif model == 'VSSBJSCC_w/_SA':
        ...
        elif model == 'VSSBJSCC_w/_SAandRA':
            snr_cuda = torch.tensor(snr, dtype=torch.float).to(device)
            rate_cuda = torch.tensor(rate, dtype=torch.float).to(device)
            snr_batch = snr_cuda.unsqueeze(0).expand(B, -1)
            rate_batch = rate_cuda.unsqueeze(0).expand(B, -1)
...
# Backward-compatible alias for old imports.
SwinJSCC_Encoder = VSSBJSCC_Encoder
